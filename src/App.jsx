...
const BRANDS = [
  { id:"b1", name:"Bullpadel", flag:"🇪🇸", logo:"🐂", tagline:"מהמותגים המובילים בעולם — נבחר על ידי מקצוענים רבים", featured:false, accent:"#c8a96e",
    models:[{name:"Hack 03",price:"650"},{name:"Vertex 04",price:"900"},{name:"Flow Light",price:"420"}], url:"https://www.bullpadel.com" },
  { id:"b2", name:"NOX", flag:"🇪🇸", logo:"⚡", tagline:"חדשנות וטכנולוגיה — המחבט של אגוסטין טאפיה", featured:false, accent:"#35e0ff",
    models:[{name:"AT10 Genius",price:"980"},{name:"ML10 Pro Cup",price:"720"}], url:"https://www.noxsport.com" },
  { id:"b3", name:"adidas Padel", flag:"🇩🇪", logo:"🎾", tagline:"ביצועים וסטייל בכל מגרש", featured:false, accent:"#dce8ff",
    models:[{name:"Metalbone 3.3",price:"890"},{name:"Adipower",price:"760"}], url:"https://www.adidas.com" },
  { id:"b4", name:"HEAD Padel", flag:"🇦🇹", logo:"🟦", tagline:"דיוק וכוח — מבית מותג הטניס המוביל", featured:false, accent:"#3a83ff",
    models:[{name:"Speed Motion",price:"820"},{name:"Extreme One",price:"690"}], url:"https://www.head.com" },
  { id:"b5", name:"Babolat", flag:"🇫🇷", logo:"🔷", tagline:"שליטה ותחושה למשחק מדויק", featured:false, accent:"#c8d8f0",
    models:[{name:"Technical Veron",price:"850"},{name:"Air Veron",price:"640"}], url:"https://www.babolat.com" },
  { id:"b6", name:"SIUX", flag:"🇪🇸", logo:"🔥", tagline:"מותג ספרדי אהוב במחירים נגישים", featured:false, accent:"#8aa0c0",
    models:[{name:"Diablo",price:"560"},{name:"Pegasus",price:"480"}], url:"https://www.siux.com" },
];
...
function BrandsSection({t}) {
  const sorted = [...BRANDS].sort((a,b)=> (b.featured?1:0)-(a.featured?1:0));
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:18,alignItems:"start"}}>
      {sorted.map(b => (
        <Card3D key={b.id}>
          <Glass style={{borderRadius:3,padding:0,display:"flex",flexDirection:"column",height:"100%",position:"relative",overflow:"hidden",border:b.featured?"1px solid rgba(200,169,110,0.5)":`1px solid ${BORDER}`,boxShadow:b.featured?"0 0 30px rgba(200,169,110,0.13)":"none"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:b.featured?"linear-gradient(90deg,#c8a96e,#e8c88a)":`linear-gradient(90deg,${b.accent},transparent)`}} />
            {b.featured && <div style={{position:"absolute",top:10,insetInlineEnd:10,background:"linear-gradient(135deg,#c8a96e,#e8c88a)",color:"#04080f",fontSize:9,fontWeight:800,padding:"3px 9px",borderRadius:20,letterSpacing:1}}>★ {t.brandFeatured}</div>}
            <div style={{padding:"26px 18px 0",textAlign:"center"}}>
              <div style={{fontSize:46,marginBottom:8}}>{b.logo}</div>
              <h3 style={{fontWeight:800,fontSize:19,marginBottom:2,color:SILVER}}>{b.name} <span style={{fontSize:15}}>{b.flag}</span></h3>
              <p style={{color:DIM,fontSize:12,lineHeight:1.5,margin:"8px 0 0",fontWeight:300,minHeight:34}}>{b.tagline}</p>
            </div>
            <div style={{padding:"16px 18px 0"}}>
              <div style={{borderTop:`1px solid ${BORDER}`,paddingTop:12}}>
                {b.models.map((m,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span style={{color:"#aebfd8",fontSize:13,fontWeight:500}}>🎾 {m.name}</span>
                    <span style={{color:b.featured?GOLD:SILVER,fontSize:13,fontWeight:700}}>₪{m.price}</span>
                  </div>
                ))}
              </div>
            </div>
            <div style={{padding:"6px 18px 18px",marginTop:"auto"}}>
              <a href={b.url} target="_blank" rel="noopener noreferrer" style={{display:"block",textDecoration:"none"}}>
                <button style={{width:"100%",padding:"9px 0",fontSize:12,background:b.featured?"linear-gradient(135deg,#c8a96e,#e8c88a)":"rgba(180,210,255,0.07)",color:b.featured?"#04080f":SILVER,border:b.featured?"none":`1px solid ${BORDER}`,borderRadius:3,fontWeight:700,cursor:"pointer",fontFamily:"Heebo,sans-serif",letterSpacing:1}}>{t.brandVisit} ↗</button>
              </a>
            </div>
          </Glass>
        </Card3D>

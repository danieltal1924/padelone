...
// === אייקוני SVG בזהב לכרטיס התחרות (מחליפים אימוג'ים) ===
...
// === לוגיקת תאריך לטורניר המוצג ===
const TOURNEY_DATE = "2026-06-21"; // ← עדכן כאן את תאריך הטורניר הבא (YYYY-MM-DD)
function tourneyStatus() {
  const today = new Date(); today.setHours(0,0,0,0);
  const d = new Date(TOURNEY_DATE); d.setHours(0,0,0,0);
  const diff = Math.round((d - today) / 86400000);
  return { past: diff < 0, today: diff === 0, soon: diff === 1 };
}
function tourneyBadge(lang) {
  const s = tourneyStatus();
  if (s.today) return lang === "he" ? "🔴 מתקיים היום" : "🔴 Happening today";
  if (s.soon)  return lang === "he" ? "⏰ מחר" : "⏰ Tomorrow";
  ...
  return (
    <div dir={t.dir} style={{fontFamily:"Heebo,sans-serif",background:BG,color:"#e8edf8",minHeight:"100vh",overflowX:"hidden",position:"relative"}}>
      ...
      <section id="tournaments" style={{padding:"100px 32px",maxWidth:1200,margin:"0 auto",position:"relative",zIndex:1}}>
        ...
        <div className="sline"/>
        {!tourneyStatus().past ? (<div style={{display:"flex",gap:28,flexWrap:"wrap",alignItems:"flex-start",justifyContent:"center",marginTop:20}}>
          <div style={{flex:"0 0 auto",maxWidth:340,width:"100%"}}>
            <img src={TOURNEY_POSTER} alt="טורניר פאדל רעננה" style={{width:"100%",borderRadius:10,border:"1px solid rgba(200,169,110,0.4)",boxShadow:"0 16px 44px rgba(0,0,0,0.55)",display:"block"}} />
          </div>
          <div style={{flex:"1 1 320px",maxWidth:440}}>
            ...
            <h3 style={{fontWeight:900,fontSize:28,color:SILVER,marginBottom:6}}>{lang==="he"?"טורניר פאדל – רעננה":"Padel Tournament – Raanana"}</h3>
            <p style={{color:GOLD,fontSize:15,fontWeight:600,marginBottom:18}}>{lang==="he"?"כיף · תחרות · הנאה":"Fun · Competition · Enjoyment"}</p>
            <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
              <span style={{color:"#aebfd8",fontSize:15}}><Icon name="date"/>{lang==="he"?"יום ראשון, 21.6":"Sunday, June 21"}</span>
              <span style={{color:"#aebfd8",fontSize:15}}><Icon name="clock"/>10:00–15:00</span>
              <span style={{color:"#aebfd8",fontSize:15}}><Icon name="pin"/>{lang==="he"?"פאדל טיים, רעננה":"Padel Time, Raanana"}</span>
              <span style={{color:"#aebfd8",fontSize:15}}><Icon name="racket"/>{lang==="he"?"רמות 4.0–4.5 · 16 קבוצות · מקומות מוגבלים":"Levels 4.0–4.5 · 16 teams · limited spots"}</span>
            </div>
            <div style={{background:"rgba(200,169,110,0.08)",border:"1px solid rgba(200,169,110,0.3)",borderRadius:8,padding:"14px 16px",marginBottom:22}}>
              <div style={{color:GOLD,fontSize:13,fontWeight:800,marginBottom:7}}><Icon name="trophy"/>{lang==="he"?"פרסים":"Prizes"}</div>
              <div style={{color:SILVER,fontSize:14,marginBottom:4}}><Icon name="medal"/>{lang==="he"?"מקום ראשון: 3,000 ₪ במזומן":"1st place: ₪3,000 cash"}</div>
              <div style={{color:SILVER,fontSize:14}}><Icon name="medal" color="#c0c0c0"/>{lang==="he"?"מקום שני: 2 מקומות חינם לטורניר הבא":"2nd place: 2 free spots for next tournament"}</div>
            </div>
          ...
          <p style={{textAlign:"center",color:DIM,fontSize:15,padding:"40px 0"}}>{lang==="he"?"אין תחרויות קרובות כרגע — עקבו אחרינו לעדכונים! 🎾":"No upcoming tournaments right now — follow us for updates! 🎾"}</p>
        )}
